...
// FIXED: Removed duplication, single render path, clean state management
...
export function SlotSelector({ eventId, onSelectSlot, loading, maxQuantity = 10 }: SlotSelectorProps) {
  ...
  const loadSlots = async () => {
    try {
      ...
      // Group slots by date - single source of truth
  ...
  const handleConfirmSelection = () => {
    ...
    // FIXED: Call onSelectSlot EXACTLY ONCE with both slot and quantity
  ...
  // SINGLE RENDER PATH - determine which state to show
  const renderContent = () => {
    // Loading state
    if (loadingSlots) {
      return (
    ...
    return (
      <>
        ...
        {/* Error Message (with slots present) */}
        ...
        {groupedSlots.map((group) => (
          <div key={group.date} className="bg-white rounded-lg border border-gray-200 overflow-hidden mb-6">
            {/* Date Header */}
